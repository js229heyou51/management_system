		...
		<div class="layui-card">
			<div class="layui-card-body">
				<div class="result">
					...
					<div class="action">
					 {if $msg=='权限不足'}
					 <a href="javascript:void(0);" id="href" class="layui-btn layui-btn-primary">立即关闭</a>
					 <button  class="layui-btn"><b id="close"><?php echo($wait);?></b>秒后自动关闭</button>
					 {else}
					 <a id="href" href="{$url}" class="layui-btn layui-btn-primary">立即跳转</a>
					 <button  class="layui-btn"><b id="wait"><?php echo($wait);?></b>秒后自动跳转</button>
					 {/if}
					</div>
				</div>
			</div>
		...
		{if $msg=='权限不足'}
			layui.use(['jquery','element'], function() {
				...
				window.closeOpen = function() {
					{if($type == 0)}
						layId = parent.$('.layui-tab-title li.layui-this').attr('lay-id');
						parent.$('.layui-nav-tree dd.layui-this').removeClass('layui-this');
						parent.element.tabDelete('nav', layId);
					{else}
