...
use app\common\model\GalleryCategory;
...
class GalleryCategoryService
{
	...
	public static function update($id, array $data)
	{
		...
		if (isset($data['parent_id'])) {
			...
			// 检查是否设置了子孙分类为父级
			$allChildrenIds = GalleryCategory::getAllChildrenIds($id, false);
		...
		if (isset($data['slug']) && $data['slug'] != $category->slug) {
			$exists = GalleryCategory::where('slug', $data['slug'])
				->where('id', '<>', $id)
				->find();
			if ($exists) {
	...
	public static function delete($id, $force = false)
	{
		...
		// 检查是否有子分类
		$childrenCount = GalleryCategory::where('parent_id', $id)
			->where('status', GalleryCategory::STATUS_ACTIVE)
			->count();
		...
		// 检查分类下是否有图片
		$galleryCount = Gallery::where('category_id', $id)
	...
	{
		$categories = GalleryCategory::getTree($parentId);
	...
	private static function attachGalleriesCount(&$categories)
	{
		foreach ($categories as &$category) {
			// 获取分类及其所有子分类的ID
			$categoryIds = GalleryCategory::getAllChildrenIds($category->id);
			
			// 统计图片数量
			$galleryCount = Gallery::whereIn('category_id', $categoryIds)
				->where('status', Gallery::STATUS_ACTIVE)
				->count();
			
			$category->gallery_count = $galleryCount;
	...
	{
		return GalleryCategory::getCategoryOptions(0, '', $excludeId);
	}
	...
	{
		$category = GalleryCategory::with(['parent', 'coverImage'])
			->find($id);
		...
		// 获取子分类数量
		$category->children_count = GalleryCategory::where('parent_id', $id)
			->where('status', GalleryCategory::STATUS_ACTIVE)
			->count();
		
		// 获取图片数量（仅当前分类）
		$category->gallery_count = Gallery::where('category_id', $id)
			->where('status', Gallery::STATUS_ACTIVE)
			->count();
		
		// 获取所有子孙分类图片总数
		$allCategoryIds = GalleryCategory::getAllChildrenIds($id);
		$category->total_gallery_count = Gallery::whereIn('category_id', $allCategoryIds)
			->where('status', Gallery::STATUS_ACTIVE)
			->count();
	...
	public static function updateSort(array $sortData)
	{
		foreach ($sortData as $item) {
			if (isset($item['id']) && isset($item['sort_order'])) {
				GalleryCategory::where('id', $item['id'])
	...
	{
		$category = GalleryCategory::where('id', $id)
			->where('status', GalleryCategory::STATUS_ACTIVE)
			->find();
